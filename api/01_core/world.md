...
A `World` is constructed from a `ComponentMetaRegistry` — there is **no** default
constructor. Register the component types you intend to use into the registry first,
then build the world from it.

```cpp
engine::ecs::ComponentMetaRegistry registry;
engine::RegisterCoreComponents(registry);          // Transform, Bounds, Name, ...
// + per-module registration, e.g.:
// engine::RegisterMeshRendererComponents(registry);

engine::ecs::World world(registry);
```
